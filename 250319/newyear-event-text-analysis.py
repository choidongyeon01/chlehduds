# -*- coding: utf-8 -*-
"""
Created on Wed Mar 19 09:05:14 2025
...
대부분 원소가 0 : 희소 행렬은 전체 원소 중에서 0이 아닌 원소의 비율이 매우 낮습니다

저장 공간 효율성 : 일반적인 2차원 배열로 희소 행렬을 저장하면 많은 공간이 낭비되므로,
                  0이 아닌 원소만 저장하는 효율적인 저장 방식이 필요

연산 속도 향상 : 희소 행렬의 특성을 활용하여 0이 아닌 원소에 대해서만 엔진을 수행하면
                연산 속도를 크게 향상시킬 수 있다
...
df_course.to_csv('./data/event-course-name feq.csv')
...
K-평균 알고리즘 사용시 고려사항
    ...
    초기 중심 설정 : k-means++ 알고리즘과 같은 초기 중심 설정 방법을 사용하여
                    초기 중심 설정의 영향을 줄일 수 있다
...
엘보우(Elbow) 방법
    ...
    현실세계 데이터를 다루다 보면, 이론처럼 급격하게 꺾이는 지점이 나타나지 않을 수 있다
    클러스터의 수가 너무 많으면 군집화 값을 관리하기 어려울 수도 있다
'''
# inertia 값을 시각화
...
y축의 이너셔 값 : 가장 가까운 군집의 중심과 샘플의 거리 제곱합
...
반복문을 통해 이너셔 값을 구할 때
    ...
    다음 군집부터는 거리르 계산해 군집의 중심을 이동하므로
...
silhouette_score()
    ...
    metrics : 거리 측정 방법 (기본값은 'euclidean')
...
실루엣 점수 활용
    ...
    클러스터랑 알고리즘 성능 평가 : 다양한 클러스터랑 알고리즘의 성능을 비교하는 데 사용
    
    클러스터랑 결과 시각화 : 실루엣 플롯을 통해 각 클러스터의 응집도와 분리도를 시각적으로 확인
'''
...
df.loc[df['bcluster'] == 24, ['bcluster', 'cluster', 'course']].tail(10)
...
df.loc[df['cluster'] == 28, ['bcluster', 'cluster', 'course']]
...
'''
34번 군집에 너무 많은 데이터가 몰려 있고
군집의 실루엣 계수가 음수로 나타나는 것들도 있다
